...
A thumbnail is a `ThumbDoc` = a `background` + a **flat, ordered array of layers**. Array order *is* paint order (index 0 = back). There is no nesting/grouping; every layer holds its own `x, y` in 1280×720 coordinate space. Four layer types: `TextLayer` (also used for badges/pills via its `bg` field), `ImageLayer` (uploaded/webcam photo, or a built-in Codex brand mark selected by the `brand` field), `EmojiLayer`, `ShapeLayer` (`rect | pill | bar`, where `bar` is the fake YouTube watched-progress bar). Use the `newXxxLayer()` factories — don't hand-build layer objects.
