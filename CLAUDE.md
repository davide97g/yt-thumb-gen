...
**Arrow keys are the precise counterpart to a drag** (`App.tsx`'s key handler): 1 canvas unit, 10 with Shift — the step pair Figma, Illustrator and Sketch share — and they ignore snapping on purpose. Two guards make them safe: `preventDefault` fires even with nothing selected, or the arrows scroll the stage out from under the canvas; and a focused widget matching `ARROW_OWNERS` (a Radix trigger, a tab strip) keeps its own arrows. A run of taps is **one** undo entry, which needed `nudge` to grow an optional `gesture` id — a drag is bracketed by the `select` it starts with, but a keypress is not, so without the id every nudge of the same layers would coalesce forever. `App` bumps the id after a 600 ms gap; the drag path omits it.

**Magnetic snapping is a preference, and off is a state the stage announces.** The `snapping` prop gates both `resolveSnap` and the pointerdown pass that measures the candidate lines — off, a drag costs no layout at all. Shift is unchanged and still suspends it for one drag. The toggle lives with the other two stage lenses (bottom-left of the canvas, shortcut **S**) and persists in `localStorage` under `thumb:snapping` rather than in the doc — it belongs to the person, not the design. Its chip lights up and grows a "Snap off" label when snapping is off, the inverse of the safe-area/actual-size chips: on is the default, so the state worth showing is the other one, or a layer that quietly stops aligning reads as a broken editor.

**Stage zoom magnifies the design, not the page** — `src/lib/zoom.ts` (pure, unit-tested) plus a wheel listener on the stage in `App.tsx`. A trackpad pinch reaches the browser as a wheel event with `ctrlKey` set, so left alone it zooms the whole document: rails, dock and chrome inflate along with the canvas being inspected. Over the stage that event becomes a `View` — a multiplier on whichever base scale is in force (`fitScale`, or `GRID_W` in actual-size mode) plus an offset from the stage centre — anchored on the pointer, so a pinch magnifies what you are pointing at. Four things it is not: it never touches the doc, so export, preview and the headless render are untouched; the zoom is a **size** change (`width/height`), not a `transform: scale()`, or text would blur and every pointer handler would inherit a rect its maths doesn't expect — the offset is the only transform; the floor is 1× (fit already shows the whole design, so zooming out past it shows nothing new); and the pan is clamped to the overflow, which pins the canvas centred whenever it's smaller than the stage. Once zoomed, a plain two-finger scroll pans — the only way to reach what is now off-stage — and at fit the event is left to the page rather than swallowed. **0** returns to fit, as does the chip that appears beside the readout while the magnifier is up. Touch pinch is not bound: the canvas's own pointer handlers own multi-touch, and a two-finger gesture there would fight a layer drag.
...
### Variants — competing takes on one design (`projects.variant_of`, `VariantBar`, `CompareView`)

A campaign spans platforms; a **variant set** competes within one — same design, same format, a different answer. Migration 010 adds four columns to `projects`: `variant_of` (self-FK, **ON DELETE CASCADE**), `variant_label`, `variant_won_at`, `variant_note`. A base is `variant_of IS NULL`.

**Modelling a variant as a project row rather than a new object is the whole decision.** It then inherits every affordance a design has: the preview JPEG the compare grid paints, its own version history, `render.png`, a shareable `?project=` link, MCP access, and the blob GC's reference scan (which already unions `projects`, so nothing was added there). A `project_variants` table would have duplicated half the project routes to get none of it.

The price is **one invariant spread across four queries: every list of *designs* filters `variant_of IS NULL`** — `GET /api/projects` (which gains a `variantCount` subquery instead), `GET /api/public/projects`, a campaign's design list, and its `designCount` join. Miss one and the archive doubles. `GET /api/projects/:id` deliberately does *not* filter: opening a variant by id is what makes it deep-linkable and renderable.

The relation is **one level deep by construction, not by a check** — every route resolves the id it was given to the set's base first (`variantRoot`), so a variant of a variant is a sibling. Labels start at B (the base is A, unstored, so an existing design becomes a base without being rewritten) and **fill gaps**, because "ship B" is the sentence the feature exists for and a set reading "A, C, D" after one deletion breaks it. `VARIANT_LIMIT = 8`.

`POST /api/projects/:id/promote` swaps the two documents in a transaction: the design keeps its id, name, history and published state while what it *shows* becomes the winner, and the loser lands in the slot the winner came from — so promoting again undoes it. Both sides `snapshot()` first. Both previews are cleared, each having been a picture of the other design; the editor recaptures by saving the adopted document straight back, which spends no version because the doc is byte-identical to what was just stored. **Only a base can be published** — a variant marked `is_public` still never reaches the gallery — which is why promoting exists at all rather than publishing a variant directly.

`variant_of` is CASCADE where `campaign_id` is SET NULL, and the difference is the point: a campaign is a folder of designs that stand alone, a variant with no base is orphaned junk. The archive names the count before deleting. `variant_won_at`/`variant_note` are presence-gated on `PUT` (`won`, `variantNote`) for the same reason `isPublic` is: `coalesce` can't clear a column, and a rename must not un-decide anything.

Client side, `src/lib/variants.ts` is pure and tested: `planGrid` **searches** the column count rather than fixing it (the best layout for two 16:9s is not the best for two 9:16s), and `visible()` caps live cells at `LIVE_CELLS = 4` **and reports what it dropped** — every cell is a real `ThumbCanvas`, so each take with an effect background is a WebGL context and each hydrated doc carries its images inline. `CompareView`'s second layout is the reason the feature is worth building: every take at `GRID_W`, dressed as platform cards with the design's own name as the title. The open take's cell reads the editor's **live** doc — comparing against a stale copy of what you just changed is worse than not comparing — and the fetched-doc cache is keyed by `id:updatedAt`, so the swap a promotion performs can't leave the wrong design under the right letter. Verdicts come from `checkReadability` over per-cell DOM measurements, the same way `ReadabilityPanel` measures the canvas.

Three rules in `App.tsx`: forking **saves first** (the copy is server-side, so there must be a stored document — and it makes both takes start identical); switching **saves first**, because switching away is where an unsaved edit would fall between two versions; and `saveProject()` returns the id, since a caller saving *towards* something can't wait for `projectId` state. **V** toggles compare, **1–9** switch takes (0 is still fit), Escape leaves.

`VariantSheet` + `src/lib/sheet.ts` compose the whole set into one labelled PNG — the only way this comparison leaves the editor, since "which of these three?" is asked in a chat window. Same offscreen-capture machinery as `CampaignExporter` (`left: -20000px`, two frames, `document.fonts.ready`, one document at a time), a different ending: the shots are drawn into a 2D context. `sheetLayout` scores each candidate column count on the sheet it would **actually** produce — header, padding and label strips included, since on a small sheet they are most of it — against 16:9, plus a penalty proportional to the fraction of the grid left empty, capped at four across.
...
`list_variants` / `create_variant` / `promote_variant` close the loop the other way round: an agent asked for "three title treatments" used to invent three unrelated projects, losing the fact that they answer one question. `create_variant` takes an optional `doc`, so a fork can differ immediately — that is the tool's whole point. `promote_variant`'s description tells the agent to ask first: which take ships is the user's call.
