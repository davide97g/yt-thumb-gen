...
/** "Aggiungi MCP": hands out the hosted endpoint plus a token, in the shape each client
 *  expects. The endpoint is remote, so nothing here requires a checkout or an install. */
export function AddMcpDialog({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onPointerDown={onClose}>
      <div
        className="anim-panel flex w-[min(620px,94vw)] flex-col gap-4 rounded-xl border border-border bg-card p-5 shadow-xl"
      ...
      >
        ...
        <div className="space-y-1.5">
          ...
          <pre className="max-h-56 overflow-y-auto whitespace-pre-wrap break-all rounded-lg border border-border bg-background/60 p-3 font-mono text-[11.5px] leading-relaxed">
            {snippet}
          </pre>
          {!fresh && (
